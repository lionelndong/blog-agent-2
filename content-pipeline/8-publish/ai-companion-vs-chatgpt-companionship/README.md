# Publish package — Why Specialized AI Companions Beat ChatGPT for Emotional Depth
...
2. Title: Why Specialized AI Companions Beat ChatGPT for Emotional Depth
3. Slug: ai-companion-vs-chatgpt-companionship
4. Description (≤80 chars): ChatGPT is a general-purpose assistant. It resets every conversation, has no
...
```bash
doppler run -- python .claude/skills/format-for-publish/scripts/format_for_strapi.py ai-companion-vs-chatgpt-companionship --publish
```
...
- Read time: ~10 min (computed at 220 wpm; informational only — not in payload)
- Blocks: 1
