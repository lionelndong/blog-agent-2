...
5. **Weave it in as natural prose — never stuff (anti-pattern #10; the `/draft` empty-paragraph test).** Apply the plan:
   ...
   - **Respect the existing structure and components:** don't bloat the article past what its content earns; don't duplicate a native `:::` component; don't stack visuals. If a kept keyword maps to a comparison/FAQ the article should have, extend the existing table/`:::faq` rather than starting a parallel one.
