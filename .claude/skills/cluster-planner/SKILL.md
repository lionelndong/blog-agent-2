...
3. **Per cluster, pick the keystone + supporting ring.**
   - **Keystone** = the single best member that targets the cluster's **parent topic** — the row maximizing `priority_score` among members whose keyword *is* (or is closest to) the `parent_topic`. The keystone is the deep, definitive article the cluster's internal links point to.
   ...
   - Tag every row: `cluster` (id) + `role` (`keystone`|`supporting`).
   - **Winnable-first ordering:** within a cluster, order by `winnability` then `priority_score`, so we build the rankable members before the reach ones (see `STRATEGY.md` §2 — ambition expands as DR grows).

4. **Coverage check.** For each active cluster flag: `no_keystone` (no member targets the parent topic yet → a gap to fill), `thin` (< 3 supporting members), or `healthy`.
...
- [ ] Every active cluster has exactly one `keystone` (or is flagged `no_keystone`).
