...
def strip_editor_notes(md: str) -> str:
    """Drop all editor-internal content from the markdown body.

    Catches three shapes (PLEAA-499 follow-up, 2026-05-08 — Neo flagged that
    editor notes were appearing on the live public blog):

    1. ``---\\n\\n## Editor notes …`` — the standard pipeline notes section,
       with or without a preceding horizontal rule.
    2. ``## Editor notes …`` — the heading on its own (no ``---`` before it).
    3. ``---\\n\\n<a id="editor…`` or ``---\\n\\n**Editor note`` — inline
       anchored / bold editor-note paragraphs that appear after a ``---``
       separator but before the formal ``## Editor notes`` heading. These were
       NOT stripped by the previous two patterns and leaked into published
       articles as visible text.

    All three patterns strip the match position and everything that follows.
    """
    # Pattern 1 + 3: horizontal rule followed by editor content
    m = re.search(
        r"^---\s*\n+(?:##\s+Editor|<a\s+id=[\"']editor|\*\*Editor\s+note)",
        md,
        re.MULTILINE | re.IGNORECASE,
    )
    if m:
        return md[:m.start()].rstrip() + "\n"
    # Pattern 2: editor-notes heading without a preceding ---
    m = re.search(r"^##\s+Editor notes", md, re.MULTILINE | re.IGNORECASE)
    if m:
        return md[:m.start()].rstrip() + "\n"
    return md


def strip_editor_callouts(md: str) -> str:
    """Remove ``:::editor … :::`` blocks from the body.

    ``transform_callouts`` converts ``**Editor:** …`` inline paragraphs into
    ``:::editor … :::`` fenced blocks. Those blocks are pipeline-internal
    annotations (written for the human editor reviewing the draft) — they must
    NOT reach the published article. This function removes them entirely so
    they never reach Strapi.

    Called AFTER ``transform_callouts`` in the main publish flow so both the
    pre-transform ``**Editor:** …`` and any hand-authored ``:::editor`` blocks
    are caught (PLEAA-499 follow-up, 2026-05-08).
    """
...
def main() -> None:
    ...
    # Strip :::editor blocks generated by transform_callouts (PLEAA-499 follow-up,
    # 2026-05-08: editor callouts are pipeline-internal and must not reach Strapi).
    body = strip_editor_callouts(body)
