...
4a. **Preserve every `:::component` fence intact (Ahrefs component library, do NOT render here).** The writer (`/draft`) emits the house-standard Ahrefs fences from `examples/ahrefs-components.md` — `:::sidenote`, `:::methodology`, `:::nutshell`, `:::key-takeaways`, `:::tip`, `:::note`, `:::stat`/`:::stat-group`, `:::expert`, `:::pullquote`, `:::further-reading`, `:::cta`. These pass through to the published `article.md` body **verbatim** so the blog page renderer (a SEPARATE CTO task — see the "Render contract" in `examples/ahrefs-components.md`) can style them. This stage does **not** convert fences to HTML. The QA/cleaning steps are fence-safe: `scrub_pipeline_scaffolding` only strips HTML comments + `_Visual asset:_` manifest captions (a `:::fence` contains neither), and `assert_no_pipeline_leak` matches only high-precision internal vocabulary (ticket refs, `content-pipeline/` paths, beat-spec/keyword-research terms) that never appears inside a legitimate component fence — so a `:::stat value="68%" …` block survives untouched.
...
4d. **Handle DEFERRED visuals (do NOT fail on leftover placeholders).** Visuals are currently DEFERRED across the pipeline: the visuals stage leaves typed placeholders, generates nothing, and does not gate. So a raw `[VISUAL:...]` (or legacy `[SCREENSHOT:...]`) reaching this stage is EXPECTED — the old hard-fail gate is **removed**. Instead the script converts each remaining placeholder into an HTML comment `<!-- VISUAL-TODO: ...original... -->`: the marker is retained in the published `article.md` source (so the future visuals agent can find every spot) but renders invisibly on the live site. This conversion runs **after** the QA-comment scrub, so these `VISUAL-TODO` comments are intentionally preserved into the output.
...
- [ ] All `:::component` fences (`:::sidenote` / `:::methodology` / `:::nutshell` / `:::key-takeaways` / `:::stat` / `:::stat-group` / `:::expert` / `:::pullquote` / `:::further-reading` / `:::cta` / `:::tip` / `:::note`) preserved verbatim in the body — NOT converted to HTML, NOT stripped by QA/cleaning
