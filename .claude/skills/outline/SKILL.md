...
6. **For each H2, write:**
   ...
   - **Visuals — NEED-DRIVEN, not a quota (governed by `templates/visual-strategy.md`).** Plan a `[VISUAL:]` for a section **only when that section needs to SHOW the reader value that text + native `:::` components cannot** — real proof, a data trend, a real example, or a concept/flow. There is **NO "image every N words"** and **no word-count interval**: most sections get none, some get one, a rare data-dense one gets two. **If you can't name the concrete value a visual adds *right here*, don't plan it.** Space them sensibly — never stack two back-to-back. (We carry more visuals than older posts only where those posts *missed* a real value-moment — fix the misses, never manufacture filler.)
     - **Value-first ~80/20 — mostly the reader's world, not us.** Per the Ahrefs evidence in `visual-strategy.md`, ~80% of visuals should show a **third party / the category**: a competitor companion app, a Google SERP for the problem, a Reddit/forum thread, an X/LinkedIn post, a real review, a real external artifact — and only ~20% our own product. **Reserve our-product shots for moments the post is genuinely on-topic about Pleasur.ai** (e.g. a capability we uniquely solve). The same method applies to **any** tool a post covers — if the post is about another product, screenshot and annotate *that* product. A reader who sees a competitor's UI, a real SERP, or a real thread gets value whether or not they ever buy.
     ...
     - **Each planned visual names three things: its PURPOSE, its value-first SOURCE (prefer third-party/world), and its TYPE.** Format:
       ```
       **Visuals:**
         Visual 1: {type: external, sub: competitor-ui, source: Character.AI chat showing the memory failure, purpose: prove the forgetting problem on a real rival, self_or_third: third-party}
         Visual 2: {type: chart, data: research.<KEY-THAT-EXISTS>, purpose: show the price-per-action gap, self_or_third: third-party-data}
         Visual 3: {type: screenshot, target: chat, what: the memory panel recalling a fact unprompted, purpose: show the capability WE uniquely solve, self_or_third: self (on-topic)}
       ```
